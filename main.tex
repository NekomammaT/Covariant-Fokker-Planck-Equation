...
\bae{
    ...
    =:&\,\int\dd^4x\left[Q^I_X\delta\tilde{\varphi}_{XI}+\frac{1}{2}Q_X^I\delta\Lambda_{XYIJ}Q^J_Y\right],
}
where
\bae{
...
\bae{
    \bce{
        \dps
        \delta\tilde{\varphi}_{QI}(t,\mathbf{x})=\int\frac{\dd^3k}{(2\pi)^3}\ee^{i\mathbf{k}\cdot\mathbf{x}}\delta(t-t_\sigma(k))\delta\varphi_{QI}(t,\mathbf{k})
        :=-\int\frac{\dd^3k}{(2\pi)^3}\ee^{i\mathbf{k}\cdot\mathbf{x}}\delta(t-t_\sigma(k))D\varpi_I(t,\mathbf{k}), \\ 
        \dps
        \delta\tilde{\varphi}_{\tilde{P}I}(t,\mathbf{x})=\int\frac{\dd^3k}{(2\pi)^3}\ee^{i\mathbf{k}\cdot\mathbf{x}}\delta(t-t_\sigma(k))\delta\varphi_{\tilde{P}I}(t,\mathbf{k})
        :=\int\frac{\dd^3k}{(2\pi)^3}\ee^{i\mathbf{k}\cdot\mathbf{x}}\delta(t-t_\sigma(k))\delta\varphi^I(t,\mathbf{k}), \\
        \dps
        \delta\Lambda_{QQIJ}(t,\mathbf{x})=\Gamma^K_{IJ}(t,\mathbf{x})\int\frac{\dd^3k}{(2\pi)^3}\ee^{i\kdx}\delta(t-t_\sigma(k))D\varpi_K(t,\mathbf{k}).
    }
}
...
\bae{
    S=S^{(0)}+\int\dd^4x\left[Q_X^I\delta\tilde{\varphi}_{XI}+\frac{1}{2}Q^I_X(\Lambda_{XYIJ}+\delta\Lambda_{XYIJ})Q^J_Y\right].
}
...
\bae{
    ...
    &\quad\times\int\scrD Q^{Ia}_X\exp\left[i\int\dd^4x(\delta\tilde{\varphi}_{XIa}+\scrJ_{XIa})Q^{Ia}_X+\frac{i}{2}\int\dd^4x\dd^4y\,Q^{Ia}_X(\tilde{\Lambda}_{XYIJab}+\delta\tilde{\Lambda}_{XYIJab})Q^{Jb}_Y\right],
}
...
\bae{
    &Z[\calJ^{Ia}_X,\scrJ_{XIa}]=\frac{\calN}{\Det(1+\tilde{\Lambda}^{-1}\delta\tilde{\Lambda})}\int\scrD\delta\varphi_{XIa}\ee^{i(S^{(0)+}-S^{(0)-}+\int\dd^4x\scrJ^{Ia}_X\delta\tilde{\varphi}_{XIa})} \nonumber \\
    &\quad\times\exp\left[-\frac{i}{2}\int\dd^4x\dd^4y\,(\delta\tilde{\varphi}_{XIa}+\scrJ_{XIa})(\tilde{\Lambda}+\delta\tilde{\Lambda})^{-1}{}_{XY}{}^I{}_J{}^a{}_b(\delta\tilde{\varphi}_Y^{Jb}+\scrJ_Y^{Jb})\right].
}
...
Therefore we obtain
\bae{
    Z[\calJ^{Ia}_X,\scrJ_{XIa}]\simeq&\,\frac{\calN}{\Det(1+\tilde{\Lambda}^{-1}\delta\tilde{\Lambda})}\exp\left[i\left(S^{(0)+}-S^{(0)-}+\int\dd^4x\calJ_X^{Ia}\braket{\delta\tilde{\varphi}_{XIa}}\right)\right. \nonumber \\
    &\left.-\frac{1}{2}\int\dd^4x\dd^4y\,(\braket{\delta\tilde{\varphi}_{XIa}}+\scrJ_{XIa})(\tilde{\Lambda}+\delta\tilde{\Lambda})^{-1}{}_{XY}{}^I{}_J{}^a{}_b(\braket{\delta\tilde{\varphi}_Y^{Jb}}+\scrJ_Y^{Jb})\right] \nonumber \\
...
where
\bae{
    \braket{Q_X^{Ia}}=\frac{\delta W}{\delta\scrJ_{XIa}}=-\int\dd^4y\,(\tilde{\Lambda}+\delta\tilde{\Lambda})^{-1}{}_{XY}{}^I{}_J{}^a{}_b(\braket{\delta\tilde{\varphi}_Y^{Jb}}+\scrJ_Y^{Jb}).
}
...
\bae{
    ...
    &+\frac{1}{2}\int\dd^4x\dd^4y\braket{Q_X^{Ia}}(\tilde{\Lambda}+\delta\tilde{\Lambda})_{XYIJab}\braket{Q_Y^{Jb}}+\int\dd^4x\braket{\delta\tilde{\varphi}_{XIa}}\braket{Q_X^{Ia}}.
}
The vacuum expectation value $\braket{Q}$ is given by the stationary point of the effective action as
\bae{
    \frac{\delta\Gamma}{\delta\braket{Q_X^{Ia}}}=0, \quad \Leftrightarrow \quad \braket{Q_X^{Ia}}=-\int\dd^4y(\tilde{\Lambda}+\delta\tilde{\Lambda})^{-1}{}_{XY}{}^I{}_J{}^a{}_b\braket{\delta\tilde{\varphi}_X^{Jb}}
}
...
\bae{\label{eq: GammaIR}
    \Gamma_\IR&\simeq S^{(0)+}-S^{(0)-}+i\Tr(\tilde{\Lambda}^{-1}\delta\tilde{\Lambda})-\frac{1}{2}\int\dd^4x\dd^4y\braket{\delta\tilde{\varphi}_{XIa}}\tilde{\Lambda}^{-1}{}_{XY}{}^I{}_J{}^a{}_b\braket{\delta\tilde{\varphi}_Y^{Jb}} \nonumber \\
...
$\tilde{\Lambda}^{-1}$ is nothing but the closed-time-path ordered two point function of $Q$ as
\bae{
	\braket{T_C\hat{Q}_X^{Ia}(x)\hat{Q}_Y^{Jb}(y)}=\left.\frac{1}{i}\frac{\delta}{\delta\scrJ_{XIa}(x)}\frac{1}{i}\frac{\delta}{\delta\scrJ_{YJb}(y)}Z\right|_{\calJ=\scrJ=0}=i\tilde{\Lambda}^{-1}{}_{XY}{}^{IJab}(x,y),
}
where the time ordering is defined by
\bae{
	T_C\hat{Q}_{X}^{Ia}(x)\hat{Q}_Y^{Jb}(y)=
	\bce{
		\dps
		\theta(t-t^\prime)\hat{Q}_{X}^{Ia}(x)\hat{Q}_Y^{Jb}(y)+\theta(t^\prime-t)\hat{Q}_Y^{Jb}(y)\hat{Q}_X^{Ia}(x), & (a=+,\,b=+), \\
		\dps
		\hat{Q}_Y^{Jb}(y)\hat{Q}_X^{Ia}(x), & (a=+,\,b=-), \\
		\dps
		\hat{Q}_X^{Ia}(x)\hat{Q}_Y^{Jb}(y), & (a=-,\,b=+), \\
		\dps
		\theta(t^\prime-t)\hat{Q}_{X}^{Ia}(x)\hat{Q}_Y^{Jb}(y)+\theta(t-t^\prime)\hat{Q}_Y^{Jb}(y)\hat{Q}_X^{Ia}(x), & (a=-,\,b=-).
	}
}
...
the explicit form of $\tilde{\Lambda}^{-1}$ is given by
\bae{
...
On the other hand, $\delta S_\IA^\mathrm{det}$ is simply summarized by considering only $\delta\Lambda_{QQ}$ as
\bae{
	\delta S_\IA^\mathrm{det}=\frac{1}{2}\int\dd^4x\int\frac{\dd^3k}{(2\pi)^3}\frac{\dd^3k^\prime}{(2\pi)^3}\Re[Q_\alpha^I(t,k)Q_\alpha^J(t,k)]\Gamma_{IJ}^K(t,\mathbf{x})
	\ee^{i\mathbf{k}^\prime\cdot\mathbf{x}}\delta(t-t_\sigma(k^\prime))D\varpi_K^\Delta(t,\mathbf{k}^\prime).
}
We here only keep $D\varpi^\Delta$ term and omit $D\bar{\varpi}$ because we later only vary $\Delta$-modes.
\Blue{$\delta(t-t_\sigma(k^\prime))$ rather should be $\delta(t-t_\sigma(k))$ (to be modified). If one adopts the latter delta function, $\delta S_\IA^\mathrm{det}$ reads}
\bae{
	\delta S_\IA^\mathrm{det}=\frac{1}{2}\int\dd^4x\calP_{QQ}{}^{IJ}(k_\sigma)\Gamma^K_{IJ}D\varpi_K^\Delta.
}

Therefore one obtains the covariant EoM for $\varphi$ as
\bae{
	\left.\frac{\delta S^{(0)+}-\delta S^{(0)-}+\delta S_\IA^\mathrm{det}+\delta S_\eff^\xi}{\delta\varpi_I^\Delta}\right|_{\varphi^\Delta=\varpi^\Delta=0}=0, \quad \Leftrightarrow
	\quad \dot{\varphi}^I+\frac{1}{2}\Gamma^I_{JK}\calP_{QQ}{}^{JK}=\frac{N_\IR}{a^3}\varpi^I+\xi_Q^I.
}
\YT{$\varpi$'s EoM will be obtained}.
