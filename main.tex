...
\bae{\label{eq: quadratic expansion}
    \bce{
        \dps
        {\dphi^I}^{[1]}=Q^I, \\
        \dps
...
However vectors can be mapped to a different point by the parallel transport. Each term in the action is a scalar at $\phi^I$, which is conserved under this transport. Therefore the action can be rewritten in terms of vectors at the IR point $\varphi^I$ and then its covariance will be much clearer.
We here discuss this approach.
See Fig.~\ref{fig: QPdef} for our vector notations.


\bfe{width=\hsize}{fig/QPdef.pdf}{Schematic image of the definition of vectors. Each vector is mapped to the same color one by the parallel transport.}{fig: QPdef}


The IR mode $\varphi^I$ is in general different from the original field $\phi^I$ and thus points a different position on the inflaton's manifold. They can be connected by one geodesic line with an affine parameter $\lambda$. Let $\lambda=0$ correspond with $\varphi^I$ and $\lambda=1$ correspond with $\phi^I$. The geodesic line is along with a parallel transported vector $\dd\phi^I/\dd\lambda$. That is, it satisfies
\bae{\label{eq: lambda line}
    D_\lambda\dif{\phi^I}{\lambda}=\dif{^2\phi^I}{\lambda^2}+\Gamma^I_{JK}\dif{\phi^J}{\lambda}\dif{\phi^K}{\lambda}=0, \quad \Leftrightarrow \quad \dif{^2\phi^I}{\lambda^2}=-\Gamma^I_{JK}\dif{\phi^J}{\lambda}\dif{\phi^K}{\lambda}.
}
Then we can define the covariant UV mode of $\phi^I$ by
\bae{
    Q^I:=\left.\dif{\phi^I}{\lambda}\right|_{\lambda=0},  
}
which specifies the ``difference" between the original field $\phi^I$ and its IR mode $\varphi^I$.

The conjugate momentum $\pi_I$ is a covector at $\phi^I$, while its IR mode $\varpi_I$ should be defined at $\varphi_I$. Thus they cannot be compared directly. Instead of the original momentum $\pi_I$, one can consider a covector $\pi_I^\parallel$ at $\varphi^I$, which is parallel transported along the $\lambda$-line.
Then $\pi_I^\parallel$ can be compared with $\varpi_I$ and their difference is also a covariant vector. It can be a good representative of the covariant UV mode of $\pi_I$:\footnote{In this subsection, we use non-tilded $P_I$ for the covariant vector contrary to other parts of these notes.}
\bae{
    P_I:=\pi_I^\parallel-\varpi_I.
}
\YT{Compare it with the previous $\tilde{P}$ by explicitly writing $\pi^\parallel$.}
The parallel transport conserves a scalar product. Therefore, e.g., the kinetic term in the Hamiltonian can be easily expressed as
\bae{
    \pi_I\pi^I|_{\lambda=1}=\pi_I^\parallel\pi^I_\parallel|_{\lambda=0}=(\varpi_I+P_I)(\varpi^I+P^I).
}
It is exact without any perturbative expansion.

The scalar potential $V$ can be also clearly expressed. Its expansion should be rewritten in a covariant way as
\bae{
    V|_{\lambda=1}&=\left(V+\dif{V}{\lambda}+\frac{1}{2}\dif{^2V}{\lambda^2}+\frac{1}{6}\dif{^3V}{\lambda^3}+\cdots\right)_{\lambda=0} \nonumber \\
    &=\left[V+V_I\dif{\phi^I}{\lambda}+\frac{1}{2}\dif{}{\lambda}\left(V_I\dif{\phi^I}{\lambda}\right)+\frac{1}{6}\dif{}{\lambda}\left(\dif{}{\lambda}\left(V_I\dif{\phi^I}{\lambda}\right)\right)+\cdots\right]_{\lambda=0} \nonumber \\
    &=V+V_IQ^I+\frac{1}{2}V_{I;J}Q^IQ^J+\frac{1}{6}V_{I;JK}Q^IQ^JQ^K+\cdots.
}
The ADM variables $\calN$ and $\beta^i$ are also scalars but independent of $\phi^I$ and $\pi_I$ before they are integrated out. Thus one defines their UV modes by the direct difference as
\bae{
    \alpha:=\calN-\calN_\IR, \qquad a^{-2}\partial_i\psi:=\beta^i-0.
}
...
\bae{
    \dif{\pi_I^\parallel}{\lambda}=\Gamma_{IJ}^K\pi_K^\parallel\dif{\phi^J}{\lambda}, \qquad \dif{^2\pi_I^\parallel}{\lambda^2}=\tilde{B}^J_{IKL}\pi_J^\parallel\dif{\phi^K}{\lambda}\dif{\phi^L}{\lambda}%, \qquad \dif{^3\pi_I^\parallel}{\lambda^3}=\tilde{C}^J_{IKLM}\pi_J^\parallel\dif{\phi^K}{\lambda}\dif{\phi^L}{\lambda}\dif{\phi^M}{\lambda},
}
where
\bae{
    %\bce{
        ...
        %\dps
        %\tilde{C}^J_{IKLM}=\tilde{B}^J_{IKL,M}+\tilde{B}^S_{IKL}\Gamma^J_{SM}-\tilde{B}^J_{ISL}\Gamma^S_{KM}-\tilde{B}^J_{IKS}\Gamma^S_{LM}.
    %}
}
...
\bae{\label{eq: pi parallel expansion}
    \pi_I^\parallel|_{\lambda=0}&=\left(\pi_I^\parallel-\dif{\pi_I^\parallel}{\lambda}+\frac{1}{2}\dif{^2\pi_I^\parallel}{\lambda^2}%-\frac{1}{6}\dif{^3\pi_I^\parallel}{\lambda^3}
...
\bae{
    ...
    &\qquad\qquad +\frac{1}{6}\left(\dot{\varphi}^I-\calN_\IR\varpi^I\right)\left(3R_{IJK}{}^LQ^JQ^K\tilde{P}_{K,\kappa}+R_{IJK}{}^M{}_{;L}\varpi_MQ^JQ^KQ^L\right) \nonumber \\
    &\qquad\qquad\left. +\left(\frac{1}{6}-\kappa\right)R^L{}_{IJK}\varpi_LQ^IQ^JD_tQ^K \right].
}
...
\bae{
    ...
    &\qquad\qquad\qquad \left. +\frac{1}{6}R^L{}_{IJ}{}^M{}_{;K}\varpi_L\varpi_MQ^IQ^JQ^K\right)\nonumber \\
    &\qquad\qquad +\frac{1}{6}\left(\dot{\varphi}^I-\calN_\IR\varpi^I\right)\left(3R_{IJK}{}^LQ^JQ^K\left(\tilde{P}_{K,\alpha}-\frac{\alpha}{\calN_\IR}\varpi_K\right)+R_{IJK}{}^M{}_{;L}\varpi_MQ^JQ^KQ^L\right) \nonumber \\
    &\qquad\qquad\left. +\left(\frac{1}{6}-\kappa\right)R^L{}_{IJK}\varpi_LQ^IQ^JD_tQ^K \right].
}
...
\bae{
    ...
    B_{IJK}=&-\frac{\calN_\IR^4}{4\Mpl^2\calH^3}\varpi_IZ_J\varpi_K\left(1-\frac{(\bfk_2\cdot\bfk_3)^2}{k_2^2k_3^2}\right)-\frac{\calN_\IR}{\Mpl^2\calH}Z_IG_{JK}\frac{\bfk_1\cdot\bfk_2}{k_1^2}-\frac{\calN_\IR^3}{4\Mpl^4\calH^2}\varpi_I\varpi_J\varpi_K \nonumber \\
    &+(1+2\kappa)\calN_\IR R_{KIJ}{}^L\varpi_L, \\
    C_{IJK}=&\frac{\calN_\IR^4}{8\Mpl^4\calH^3}\varpi_I\varpi_J\varpi_K\left(1-\frac{(\bfk_1\cdot\bfk_2)^2}{k_1^2k_2^2}\right)+\frac{\calN_\IR^2}{\Mpl^2\calH}\varpi_IG_{JK}\frac{\bfk_1\cdot\bfk_3}{k_1^2}-\frac{\calN_\IR^2}{2\Mpl^2\calH}G_{IJ}\varpi_K.
}
Bold indices $\bfI$, $\bfJ$, and $\bfK$ are defined in the same way with Ref.~\cite{Butchers:2018hds}. It is equivalent to their cubic action if $\kappa=1/6$.
