...
Then $\pi_I^\parallel$ can be compared with $\varpi_I$ and their difference is also a covariant vector. It can be a good representative of the covariant UV mode of $\pi_I$:\footnote{In this subsection, we use non-tilded $P_I$ for the covariant vector contrary to other parts of this notes.}
\bae{
...
\bae{
    ...
    &\qquad\qquad\left.-\frac{a\Mpl^2}{(\calN_\IR+\alpha)^2}\psi_i\left(\alpha_i(\psi_{jj}-2a^2\calH)-\alpha_j\psi_{ij}\right)\right]
}
\bae{
...
\bae{
	...
	&\qquad\left.+\frac{1}{3}\left(\dot{\varphi}^I-\frac{\calN_\IR}{a^3}\varpi^I\right)R_{IJK}{}^L\varpi_LQ^JQ^K\right], \\
	S^{[3]}&=\int\dd^4x\left[\right]
}
